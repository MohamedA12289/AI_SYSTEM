      ...
      <aside className="w-14 flex-shrink-0 bg-sidebar border-r border-sidebar-border flex flex-col h-screen sticky top-0">
        ...
        <nav className="flex-1 flex flex-col items-center gap-1 py-2 px-1.5">
          ...
          {projects.slice(0, 6).map((p) => (
            <button key={p.id} onClick={() => navigate(`/project/${p.id}/chat`)} title={p.name}
              className={`w-9 h-9 rounded-lg flex items-center justify-center text-xs font-semibold transition-colors ${location.pathname.startsWith(`/project/${p.id}`) ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent/50"}`}>{p.name.charAt(0)}</button>
          ))}
          ...
          <div className="w-5 h-px bg-sidebar-border my-1" />
          <button onClick={() => navigate("/self-upgrade/chat")} className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${location.pathname.startsWith("/self-upgrade") ? "bg-warning/10 text-warning" : "text-sidebar-foreground hover:bg-sidebar-accent/50"}`}><Zap className="w-4 h-4" /></button>
        </nav>
    ...
    <aside className="w-56 flex-shrink-0 bg-sidebar border-r border-sidebar-border flex flex-col h-screen sticky top-0">
      ...
      <nav className="flex-1 overflow-y-auto scrollbar-thin py-2 px-2 space-y-0.5">
        ...
        <div className="pt-3">
          ...
          {historyOpen && <div className="mt-1 space-y-2">{(['today','yesterday','older'] as const).map((group) => grouped[group].length ? <div key={group}><div className="px-2.5 text-[10px] uppercase tracking-wider text-muted-foreground/70 mb-1">{group}</div><div className="space-y-0.5">{grouped[group].slice(0,6).map((h) => <button key={h.id} onClick={() => h.projectId ? navigate(`/project/${h.projectId}/chat`) : navigate('/')} className="w-full text-left px-2.5 py-2 rounded-lg hover:bg-sidebar-accent/50 transition-colors"><div className="text-[12px] text-foreground truncate">{h.title}</div><div className="text-[10px] text-muted-foreground truncate">{h.preview}</div></button>)}</div></div> : null)}</div>}
        </div>
        <div className="pt-3">
          <button onClick={() => navigate('/self-upgrade/chat')} className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[13px] transition-colors ${location.pathname.startsWith('/self-upgrade') ? 'bg-warning/10 text-warning font-medium' : 'text-sidebar-foreground hover:bg-sidebar-accent/50 hover:text-foreground'}`}><Zap className="w-4 h-4" />Self-Upgrade</button>
        </div>
      </nav>
