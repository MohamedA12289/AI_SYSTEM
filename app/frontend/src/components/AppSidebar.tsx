...
import { Home, Settings, Plus, ChevronRight, ChevronLeft, Zap, MessageCircle, Cpu } from "lucide-react";
...
export function AppSidebar({ collapsed, onToggle, projects = [], chatHistory = [] }: Props) {
  ...
  const isInProjectThread = location.pathname.includes("/thread/");

  const modelLabel = activeModel ? (activeModel.split("/").pop()?.split(":")[0] ?? activeModel) : "";
    ...
    return (
      <aside className="w-14 flex-shrink-0 bg-sidebar border-r border-sidebar-border flex flex-col h-screen sticky top-0">
        ...
        <nav className="flex-1 flex flex-col items-center gap-1 py-2 px-1.5">
          <button onClick={() => navigate("/")} title="Home" className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${isActive("/") ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent/50"}`}><Home className="w-4 h-4" /></button>
          <button onClick={() => navigate("/chats")} title="Chats" className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${isActive("/chats") ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent/50"}`}><MessageCircle className="w-4 h-4" /></button>
          <div className="w-5 h-px bg-sidebar-border my-1" />
          {projects.slice(0, 6).map((p) => (
            <button key={p.id} onClick={() => navigate(`/project/${p.id}`)} title={p.name}
              className={`w-9 h-9 rounded-lg flex items-center justify-center text-xs font-semibold transition-colors ${location.pathname.startsWith(`/project/${p.id}`) ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent/50"}`}>{p.name.charAt(0).toUpperCase()}</button>
          ))}
          <button onClick={() => navigate("/new-project")} title="New Project" className="w-9 h-9 rounded-lg flex items-center justify-center text-sidebar-foreground hover:bg-sidebar-accent/50 transition-colors"><Plus className="w-4 h-4" /></button>
          <div className="w-5 h-px bg-sidebar-border my-1" />
          <button onClick={() => navigate("/self-upgrade")} title="Self-Upgrade" className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${location.pathname.startsWith("/self-upgrade") ? "bg-warning/10 text-warning" : "text-sidebar-foreground hover:bg-sidebar-accent/50"}`}><Zap className="w-4 h-4" /></button>
        </nav>
        <div className="flex flex-col items-center gap-1 pb-3 px-1.5">
          {modelLabel && (
            <button onClick={() => navigate("/settings")} title={`Model: ${modelLabel}`} className="w-9 h-9 rounded-lg flex items-center justify-center text-sidebar-foreground hover:bg-sidebar-accent/50 transition-colors">
              <Cpu className="w-3.5 h-3.5" />
            </button>
          )}
          <button onClick={() => navigate("/settings")} title="Settings" className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${isActive("/settings") ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent/50"}`}><Settings className="w-4 h-4" /></button>
          <button onClick={onToggle} title="Expand sidebar" className="w-9 h-9 rounded-lg flex items-center justify-center text-sidebar-foreground hover:bg-sidebar-accent/50 transition-colors"><ChevronRight className="w-3.5 h-3.5" /></button>
        </div>
      </aside>
    );
  }
  ...
  return (
    <aside className="w-56 flex-shrink-0 bg-sidebar border-r border-sidebar-border flex flex-col h-screen sticky top-0">
      <div className="px-3 py-3.5 border-b border-sidebar-border flex items-center justify-between">
        ...
        <button onClick={onToggle} title="Collapse sidebar" className="p-1 rounded-md text-sidebar-foreground hover:bg-sidebar-accent/50 transition-colors"><ChevronLeft className="w-3.5 h-3.5" /></button>
      </div>
      <nav className="flex-1 overflow-y-auto scrollbar-thin py-2 px-2 space-y-0.5">
        <button onClick={() => navigate("/")} className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[13px] transition-colors ${isActive("/") && location.pathname === "/" ? "bg-sidebar-accent text-sidebar-accent-foreground font-medium" : "text-sidebar-foreground hover:bg-sidebar-accent/50 hover:text-foreground"}`}><Home className="w-4 h-4" />Home</button>
        <button onClick={() => navigate("/chats")} className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[13px] transition-colors ${isActive("/chats") ? "bg-sidebar-accent text-sidebar-accent-foreground font-medium" : "text-sidebar-foreground hover:bg-sidebar-accent/50 hover:text-foreground"}`}><MessageCircle className="w-4 h-4" />Chats</button>
        <div className="pt-3">
        ...
        <div className="pt-3">
          <button onClick={() => navigate('/self-upgrade')} className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[13px] transition-colors ${location.pathname.startsWith('/self-upgrade') ? 'bg-warning/10 text-warning font-medium' : 'text-sidebar-foreground hover:bg-sidebar-accent/50 hover:text-foreground'}`}><Zap className="w-4 h-4" />Self-Upgrade</button>
        </div>
      </nav>
      <div className="p-2 border-t border-sidebar-border space-y-0.5">
        {modelLabel && (
          <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] text-muted-foreground" title={`Active model: ${activeModel}`}>
