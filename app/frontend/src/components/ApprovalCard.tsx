...
export function ApprovalCard({ message, isSelfUpgrade, projectName, onResolved }: Props) {
        ...
        {status === "pending" && (
          <div className="flex gap-2">
            <button onClick={async () => {
              if (projectName && message.approvalId) {
                try { await api.approvals.approve(projectName, message.approvalId); } catch {}
              }
              setStatus("approved");
              onResolved?.();
            }}
            ...
            <button onClick={async () => {
              if (projectName && message.approvalId) {
                try { await api.approvals.reject(projectName, message.approvalId); } catch {}
              }
              setStatus("rejected");
              onResolved?.();
            }}
