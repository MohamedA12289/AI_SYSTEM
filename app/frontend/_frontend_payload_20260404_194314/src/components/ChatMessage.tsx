import { Bot, User, Brain, AlertTriangle, Terminal, FileCode, Loader2 } from "lucide-react";
...
export function ChatMessage({ message, isSelfUpgrade }: Props) {
  if (message.role === "approval") {
    return <ApprovalCard message={message} isSelfUpgrade={isSelfUpgrade} />;
  }

  // Rich tool result cards
  if (message.role === "tool" && message.toolType && richToolTypes.includes(message.toolType)) {
    return <ToolResultCard message={message} />;
  }

  const isUser = message.role === "user";
  const isPlanning = message.role === "planning";
  ...
  if (isTool) {
    return (
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-surface border border-border text-xs text-muted-foreground animate-fade-in">
        ...
        <span className="ml-auto text-[10px]">{message.timestamp}</span>
      </div>
    );
  }
